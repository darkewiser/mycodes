...
(function () {
    ...
    toolTip.prototype.getStyle = function (style) {
        if (this.Tip) {
            return this.Tip.style[style];
        }
    }
    ...
    _point.prototype.addTooltip = function () {
        var tw = this.wrapper.messureText(this.fontWeight, this.fontSize, this.fontFamily, this.text).width;
        this.toolTip = new toolTip(this.wrapper, this.text, tw + 20);
    }
        ...
        this.addRenderHandler(function () {
            ...
            if (this.oValue != undefined) {
                ...
                for (var i = 1; i < len; i++) {
                    ...
                    var tw = this.wrapper.messureText(this.XfontWeight, this.XfontSize, this.XfontFamily, this.labels[i]).width;
                    this.wrapper.save();


                    if (tw > this.soltLength*0.8) {
                        this.wrapper.translate(_soltPosition[i] - tw / 3, _xLableHeight - this.XfontSize);
                        this.wrapper.rotate(-16);
                    }
                    else {
                        this.wrapper.translate(_soltPosition[i] - tw / 2, 10);
                    }

                    this.wrapper.drawString(this.labels[i], 0, 0, tw, this.XfontSize + 8, this.XfontColor, this.XfontSize, this.XfontFamily, this.XfontWeight, "center", "center", true);
...
var lineChart = function (cvsId, w, h) {
    ...
    this.addConfigHandler(function () {
        var _wrapper = this.wrapper,
            ...
            _displayPointText = this.displayPointText,
            _displayTooltip = this.displayTooltip;
        ...
        for (var j = 0; j < _data.length; j++) {
            ...
            for (var i = 0, len, txt; len = _lData.length, i < len; i++) {
                ...
                _point.displayText = _displayPointText;
                ...
                (function (k) {
                    ...
                    _point.onmousemove(function (cord) {
                        ...
                        if (_displayTooltip) {
                            ...
                            this.toolTip.setStyle("borderColor", this.backColor);
                            var h = parseFloat(this.toolTip.getStyle("height"));
                            this.toolTip.move(cord.e.pageX - (cord.x - this.cx + this.toolTip.w / 2), cord.e.pageY - (cord.y - this.cy + this.r + h));
                        }
                    });
...
var heartbeatLineChart = function (params) {
    ...
    _lineChart.displayPointText = _defaults.displayPointText;
    _lineChart.displayTooltip = _defaults.displayTooltip;
    ...
    return {
        ...
        loadingCompletedCallBack:function(func){
            _lineChart.loadingCompletedCallBack(func);
        },
