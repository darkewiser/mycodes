...
(function( $ ){
    ...
    $.fn.vectorMap = function(options) {
        var defaultParams = {
			...
			color: '#fec0c3',
            hoverColor: '#d36165',//鼠标放上去颜色
            scaleColors: ['#b6d6ff', '#005ace'],
			normalizeFunction: 'linear'
		}, map;
        
        if (options == "getMap") {
            return this.data('mapObject');
        }
	...
	VectorCanvas.prototype = {
        ...
        createText: function (config) {
            ...
            try
            {
                ...
                var text = this.rootGroup.ownerDocument.createTextNode(config.name);
        ...
        applyTransformParams: function(scale, transX, transY) {
            if (this.mode == 'svg') {
                ...
                //this.rootGroup.setAttribute('transform', 'scale(1) translate(0)');
            } else {
                this.rootGroup.coordorigin = (this.width - transX-20) + ',' + (this.height - transY - 15) //(this.width-transX+20)+','+(this.height-transY-15);
    ...
    var WorldMap = function(params) {
		...
		this.rootParent = this.canvas.createGroup(false);
		this.rootParent.setAttribute("id", params.map + "rootParentGroup");

		this.rootGroup = this.canvas.createGroup(true);
       

		this.cityGroup = this.canvas.createGroup(false);
		this.cityGroup.setAttribute("style", "display:none;");
		this.cityGroup.setAttribute("id", params.map + "cityGroup");

		this.rootParent.appendChild(this.rootGroup);
		this.rootParent.appendChild(this.cityGroup);
		...
		for(var key in mapData.pathes) {
			...
			if (typeof (mapData.pathes[key].parent) != "undefined") {
			    var citypath = this.canvas.createPath({ path: mapData.pathes[key].path });
			    citypath.setFill(this.color);
			    citypath.id = 'jvectormap' + map.index + '_' + key;
			    map.countries[key] = path;
			    this.cityGroup.appendChild(citypath);
			}

		}
		if (params.makers != undefined) {

		    var makernode = this.canvas.createSvgNode('g');
		    makernode.setAttribute("style", "font-size: 4px;");

		    var makercity = this.canvas.createSvgNode('g');
		    makercity.setAttribute("style", "font-size: 2px;");

		    for (var key in mapData.pathes) {
		        try {
		            var mk = this.canvas.createText(mapData.pathes[key]);
		            if (typeof (mapData.pathes[key].parent) != "undefined") {
		                makercity.appendChild(mk);
		            }
		            else {
		                makernode.appendChild(mk);
		            }
		        }
		        catch (e) { }
		    }
		    $(this.rootGroup).append(makernode);

		    this.cityGroup.appendChild(makercity);
		}
        ...
        // set label left top value
		params.container.mousemove(function(e){
			if (map.label.is(':visible')) {
				map.label.css({
				    left: e.pageX+15,//e.pageX-15-map.labelWidth
				    top: e.pageY - 25 - map.labelHeight //e.pageY-15-map.labelHeight
				})
			}
		});
		...
		this.canvas.canvas.appendChild(this.rootParent);
	...
	WorldMap.prototype = {
        ...
        getMapData:function(key)
        {
            return this.Data.pathes[key];
        },
		
        setFocusScale: function (scale, anchorX, anchorY, isCentered) {
            var zoomStep;

            if (scale > this.zoomMax * this.baseScale) {
                scale = this.zoomMax * this.baseScale;
            } else if (scale < this.zoomMin * this.baseScale) {
                scale = this.zoomMin * this.baseScale;
            }
        ...
        setFocus: function (scale, centerX, centerY) {
            ...
            if ($.isArray(scale) || this.countries[scale]) {
                if ($.isArray(scale)) {
                    codes = scale;
                } else {
                    codes = [scale]
                ...
                for (i = 0; i < codes.length; i++) {
                    if (this.countries[codes[i]]) {
                        itemBbox = this.countries[codes[i]].getBBox();
                        if (itemBbox) {
                ...
                this.setFocusScale(scale, -centerX * this.defaultWidth, -centerY * this.defaultHeight, true);
            }
        },
        ...
        applyTransform: function (params) {
            ...
            if (params != undefined && params.cusScale != undefined) {
                this.canvas.applyTransformParams(params.cusScale, params.translateXY.X, params.translateXY.Y);
            }
            else {
